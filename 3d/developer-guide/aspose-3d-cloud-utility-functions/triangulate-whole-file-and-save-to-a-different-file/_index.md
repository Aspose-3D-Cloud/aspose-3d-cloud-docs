---
title: "Triangulate whole file and save to a different file"
type: docs
url: /triangulate-whole-file-and-save-to-a-different-file/
weight: 40
---

# **Introduction**
Aspose.3D Cloud allows you to Triangulate the whole file and save it to a different file. The Aspose.3D Cloud API provides the following methods
### **API Information**

|**API**|**Type**|**Description**|**Swagger Link**|
| :- | :- | :- | :- |
|/3d/triangulate/new|POST|Triangulate data|[PostTriangulateNew](https://apireference.aspose.cloud/3d/#/Utilities/PostTriangulateNew)|
### **cURL Example**
{{< tabs tabTotal="2" tabID="1" tabName1="Request" tabName2="Response" >}}

{{< tab tabNum="1" >}}

```java

curl -X POST "https://api.aspose.cloud/v3.0/3d/triangulate/new?name=Aspose3D.pdf&newfilename=triangle.pdf&newformat=pdf" -H "accept: application/json" -H "authorization: Bearer eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..AHT0p9Fm2fDv5veCN73VAVgE9OEvduRNXp3btWXqIFckF1sgIv6aBnxBCRtnYzFP9iw9xJm0i09LbpPA89VoxSCAIQ5FfRarOBMUz\_mDH\_tI2PO1eMd8XRdIYamAloNh0fi0wje7y5NyR41IV5VWhxMcPeYI6Q0yyVzMzmRIuTkY9ApK7sUY4rXXamWF4xgeT5spY9CFYuAZu-6Ivh2IGLqvvMPHwC28TLX4\_oDSlnocAPcwbIz\_16UUjtBC8nZWdsm\_0375wDK0h9M3oDwalu0NbnzHukarVuHuEA0ceSi1muhCC\_ZNJnqbbwlFSClr2lZdqGyeeyvcQeX2F7nwAg"

```

{{< /tab >}}

{{< tab tabNum="2" >}}

```java

 NO RESPONSE

```

{{< /tab >}}

{{< /tabs >}}
### **SDK Example**
{{< tabs tabTotal="2" tabID="4" tabName1="C#" tabName2="Java" >}}

{{< tab tabNum="1" >}}

{{< gist "aspose-cloud" "562f676098edf247d40c311d2e4414c9" "PostTriangulateNewExample.cs" >}}

{{< /tab >}}

{{< tab tabNum="2" >}}

{{< gist "aspose-cloud" "562f676098edf247d40c311d2e4414c9" "postTriangulateNewExample.java" >}}

{{< /tab >}}

{{< /tabs >}}
